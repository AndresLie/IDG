...
from iadgen_v2.generation_critic import critic_weights, score_generation
...
def _critic_record(row: dict[str, Any], references: dict[tuple[str, str], list[dict[str, str]]], phase11: dict[str, Any]) -> dict[str, Any]:
    ...
    generation_mask_path = str(row.get("generation_mask_path") or settings.get("generation_mask_path") or inpaint_mask_path)
    generation_mask_variant = str(row.get("generation_mask_variant") or settings.get("mask_variant_used") or "")
    generation_mask_role = str(
        row.get("generation_mask_role")
        or settings.get("generation_mask_role")
        or _mask_variant_role(generation_mask_variant)
    )
    generation_mask_role_warning = str(row.get("generation_mask_role_warning") or settings.get("generation_mask_role_warning") or "")
    benchmark_eval_mask_path = str(row.get("benchmark_eval_mask_path") or settings.get("benchmark_eval_mask_path") or "")
    ...
    if reference:
        ref_image = Image.open(reference["image_path"]).convert("RGB")
        ref_mask = Path(reference["mask_path"])
    ...
    return {
        ...
        "generation_mask_variant": generation_mask_variant,
        "generation_mask_role": generation_mask_role,
        "generation_mask_role_warning": generation_mask_role_warning,
        ...
        "reject_reasons": ",".join(scores.reject_reasons),
...
def _write_csv(path: Path, records: list[dict[str, Any]]) -> None:
    fields = [
        ...
        "generation_mask_variant",
        "generation_mask_role",
        "generation_mask_role_warning",
        "benchmark_eval_mask_path",
        "reference_path",
        "schema_version",
    ]
...
def _write_summary(
    ...
    role_warning_counts: dict[str, int] = {}
    for row in records:
        role = str(row.get("generation_mask_role", "") or "unknown")
        role_counts[role] = role_counts.get(role, 0) + 1
        warning = str(row.get("generation_mask_role_warning", ""))
        if warning:
            role_warning_counts[warning] = role_warning_counts.get(warning, 0) + 1
    ...
    if role_warning_counts:
        lines.append("")
        lines.append("Mask-role warnings:")
        for warning, count in sorted(role_warning_counts.items(), key=lambda item: (-item[1], item[0])):
            lines.append(f"- `{warning}`: `{count}`")
    else:
        lines.append("- Mask-role warnings: `0`")
    lines.extend(
        [
            "",
            "## Reject Reasons",
            ...
            json.dumps(
                {
                    ...
                    "weights": critic_weights(phase11),
                },
                indent=2,
                sort_keys=True,
            ),
