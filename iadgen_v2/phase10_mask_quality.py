...
def _critic_record(row: dict[str, Any], policy: str, mask_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "sample_id": f"{row.get('category', '')}/{row.get('defect_type', '')}/{image_path.name}",
...
def _write_summary(summary_path: Path, records: list[dict[str, Any]], contact_sheet: Path, csv_path: Path, jsonl_path: Path) -> None:
    ...
    lines.extend(["", "## Per-Image Winner", "", "| Sample | Best policy | Critic | Selected refinement | Morphology |", "| --- | --- | ---: | --- | --- |"])
    for sample_id in sorted(best_by_image):
        row = best_by_image[sample_id]
        lines.append(
            f"| `{sample_id}` | `{row['policy']}` | {float(row['critic_score']):.4f} | "
