...
A twenty-fourth bounded command:

- resumed all 925 checkpoint rows and incremented `resume_count` to 23;
- added the next 51 pcb3 rows and stopped at 976 unique records;
- was interrupted during Qwen inference for pcb3 `076`;
- left no uncheckpointed image or mask artifact;
- preserved the run ID, both frozen fingerprints, every durable mask role, and
  the sealed reference boundary.

A twenty-fifth bounded command:

- resumed all 976 checkpoint rows and incremented `resume_count` to 24;
- flushed 48 new rows, completing pcb3 and adding the first 24 pcb4 rows;
- was interrupted during proposal refinement for pcb4 `024`;
- left a recoverable 1,024-row `metadata.jsonl.tmp`, but interruption during
  cleanup prevented `last_run_status.json` from leaving `running` state.

A short native orphan-recovery command then:

- detected that the recorded PID was no longer live and recovered all 1,024
  flushed rows through the normal resume contract;
- completed pcb4 `024`, then received one manual interrupt during the next
  sample's model initialization;
- atomically finalized a valid 1,025-row partial checkpoint with
  `resume_count: 25`;
- left no uncheckpointed pcb4 `025` artifact and did not open official masks.

A twenty-sixth bounded command:

- resumed all 1,025 checkpoint rows and incremented `resume_count` to 26;
- added the next 45 pcb4 rows and stopped at 1,070 unique records;
- used one manual interrupt instead of a timeout escalation, allowing status
  finalization to complete normally;
- interrupted during overlay encoding for uncheckpointed pcb4 `070`;
...
| Twenty-fourth-segment rate, 51 pcb3 rows | `17.613 s/image` |
| Twenty-fourth-vs-twenty-third rate change | `-7.82%` |
| Twenty-fifth-segment rate, 48 transition rows | `18.606 s/image` |
| Twenty-fifth-vs-twenty-fourth rate change | `+5.64%` |
...
edge_fused_q850_component_2_2: 1
```
...
The next 51 pcb3 rows reproduce complete soft-mask acceptance with slightly
lower confidence and much weaker Qwen localization:

```text
soft_mask_only: 51/51 versus 25/25
mean expected IoU: 0.3518 versus 0.3735
mean conformal IoU lower bound: 0.2163 versus 0.2380
mean source disagreement: 0.3803 versus 0.4061
mean selected-mask area: 0.1122 versus 0.1200
Qwen valid localization: 37/51 versus 23/25
Qwen full-image fallback: 14/51 versus 2/25
structure profile: ring_sector 51/51
```

The same-category replication supports the soft-prior design: acceptance
survives a large increase in localization fallback while provider disagreement
and mask area decline. It also exposes a calibration weakness. Every pcb3 row
is accepted despite meaningful confidence and localization variation, so the
current disposition threshold offers no risk-coverage separation on this
category.

Across 76 pcb3 rows, all 76 are `soft_mask_only`. Mean expected IoU is `0.3589`,
mean lower bound is `0.2235`, mean disagreement is `0.3888`, and mean selected
area is `0.1148`. Sixteen rows use Qwen fallback, and all 76 remain
`ring_sector`. Locked evaluation must determine whether this is robust anomaly
coverage or uniform overconfidence on broad PCB regions.

The final 24 pcb3 rows preserve complete acceptance while becoming slightly
more conservative:

```text
soft_mask_only: 24/24 versus 76/76
mean expected IoU: 0.3470 versus 0.3589
mean conformal IoU lower bound: 0.2115 versus 0.2235
mean source disagreement: 0.3561 versus 0.3888
mean selected-mask area: 0.1029 versus 0.1148
Qwen valid localization: 21/24 versus 60/76
structure profile: ring_sector 24/24
```

Completed pcb3 contains 100 `soft_mask_only` and zero `needs_review`
decisions. Mean expected IoU is `0.3561`, mean lower bound is `0.2206`, mean
disagreement is `0.3810`, and mean selected area is `0.1119`. The final slice
confirms that the no-abstention regime persists across the full category.

The first 25 pcb4 rows are also all accepted, but enter a lower-confidence,
smaller-mask regime:
...
Compared with pcb3, pcb4 confidence and area fall while disagreement rises.
Disposition still remains uniformly permissive. Locked evaluation must test
whether the selector's risk threshold is too coarse across PCB categories or
whether both categories genuinely support reliable soft pseudo-labels.

The next 45 pcb4 rows introduce selective abstention without localization
failure:

```text
soft_mask_only: 41/45 versus 25/25
needs_review: 4/45 versus 0/25
mean expected IoU: 0.2823 versus 0.2915
mean conformal IoU lower bound: 0.1468 versus 0.1560
mean source disagreement: 0.4207 versus 0.4295
mean selected-mask area: 0.0797 versus 0.0712
Qwen valid localization: 45/45 versus 25/25
structure profile: ring_sector 44, repeated_chain 1
```

The four review rows are pcb4 `032`, `039`, `041`, and `043`. Their expected
IoU spans `0.1677` to `0.2385`, while selected area spans `0.0179` to `0.0473`.
Because Qwen is valid for every row and aggregate disagreement improves, the
abstentions are candidate/selector driven rather than localization driven.

Across 70 pcb4 rows, 66 are `soft_mask_only` and four are `needs_review`.
Mean expected IoU is `0.2856`, mean lower bound is `0.1501`, mean disagreement
is `0.4238`, and mean selected area is `0.0767`. This is the first within-PCB
risk separation observed in the external cohort, but official overlap is still
