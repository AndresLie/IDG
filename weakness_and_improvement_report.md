...
`ring_sector`. Completed pcb1 contains 73/100 soft-mask decisions, mean expected
IoU `0.2436`, mean disagreement `0.5800`, and mean area `0.0950`. Across 78
pcb2 rows, 30 are accepted as soft masks, expected IoU is `0.2106`,
disagreement is `0.4274`, and area is `0.0437`. A matched comparison shows the
next 45 pcb2 rows fall from `48.5%` to `31.1%` acceptance relative to the first
33, while Qwen validity and disagreement remain nearly unchanged. The final 22
pcb2 rows rebound to `54.5%` acceptance and complete the category at 42/100,
confirming within-category heterogeneity rather than monotonic collapse. Pcb3
completes at 100/100 soft-mask acceptance, expected IoU `0.3561`, and mean area
`0.1119`. The final 24 rows remain fully accepted while becoming slightly more
conservative. Pcb4 opens at 25/25 acceptance but lower expected IoU (`0.2915`)
...
The latest interruption exposed a second operational edge case. Forty-eight
rows were flushed to `metadata.jsonl.tmp`, but interruption during cleanup left
`last_run_status.json` marked `running` after the process exited. Native orphan
recovery correctly validated and recovered all 1,024 rows, then finalized a
1,025-row partial checkpoint after one manual interrupt. This preserves the
...
## 1g. V4.2e area-calibrated candidate result (2026-08-11)

V4.2e tested the revised hypothesis directly. It retained every V4.2d mask and
added category-free, evidence-density-ranked component unions plus seeded
support candidates capped at `10%` image area. The run was preregistered before
execution and evaluated the same five exposed sources with complete
leave-dataset-out folds.

| Dataset | V4.2d oracle | V4.2e oracle | Oracle gain | V4.2d selected | V4.2e selected |
| --- | ---: | ---: | ---: | ---: | ---: |
| BTAD | `0.3218` | `0.3353` | `+0.0136` | `0.2305` | `0.2349` |
| Kodytek | `0.4951` | `0.5063` | `+0.0112` | `0.3625` | `0.3252` |
| KSDD2 | `0.6144` | `0.6315` | `+0.0171` | `0.5274` | `0.5274` |
| MVTec development | `0.6539` | `0.6695` | `+0.0156` | `0.5811` | `0.5509` |
| VisA exposed | `0.3583` | `0.3717` | `+0.0134` | `0.1593` | `0.1593` |
| **Dataset macro** | **`0.4887`** | **`0.5029`** | **`+0.0142`** | **`0.3722`** | **`0.3595`** |

The proposal mechanism works, but not enough to promote:

- The primary macro-oracle gate (`>= +0.01`) passes, and all five sources are
  non-regressive because the pool is a strict superset.
- The VisA oracle gate fails: `+0.0134` versus the preregistered `+0.03` target.
  New candidates become oracle on `201/1200` VisA samples, but the remaining
  ceiling is still low (`0.3717`).
- Deployment fails. Refitted selected Dice regresses by `-0.0126` macro, driven
  by MVTec (`-0.0303`) and Kodytek (`-0.0373`), while VisA still abstains.
- Expected-IoU MAE worsens `0.0857 -> 0.1021`; candidate count grows
  `27,816 -> 41,638`, and elapsed time grows `2,154 -> 3,588` seconds.
- Area ratio improves only slightly (`2.539 -> 2.510`). Combining components
  does not solve broad within-component boundaries or VisA's `5.25x` selected
  area error.

A committed cache-hit rerun reproduced the model, candidate-row cache, metrics,
and generated report byte-for-byte. Runtime decreased from `3,587.50` to
`3,236.32` seconds (`1.11x`), confirming that the cache removes reconstruction
cost but leaves nested all-pairs calibration as the dominant runtime. The
started-manifest architecture fingerprint differs because the first run began
before its configured candidate-calibrator output existed and the rerun hashed
that output. Package-code and configured-input fingerprints are identical; this
is a self-referential provenance limitation, not a metric/model discrepancy.

**Decision:** keep the contract, cache, and additive family as experimental
infrastructure, but keep V4.2e disabled. Do not tune selection thresholds or add
more component-union permutations. The next candidate-quality study must target
within-component boundary precision using category-free source-consensus seeds
...
- A cache-hit rerun reproduces the cache, model, metrics, report, architecture,
  package, model, config, and split fingerprints exactly. Runtime improves
  `2,502.87 -> 2,072.77` seconds (`17.18%`), leaving nested list-ranker fitting
...
| **V4.1** | Cross-dataset score-to-mask calibration | ✅ Prototype complete; promotion gate failed | Added a hash-tracked category-agnostic pixel posterior, robust/rank features, soft localization prior, stable-normal boundary feature, Otsu floor, compact components, and broad-mask arbitration. The 1,344-image leave-dataset-out run finds an area-compliant row with macro Dice `0.3506`, precision `0.3805`, and area ratio `1.9240`, but MVTec Dice falls `0.5889 -> 0.5244` while VisA rises only `0.1593 -> 0.1769`. A clean committed rerun reproduces the model, full metric matrix, report, and manifest hashes exactly in `353.34` seconds. | Keep the posterior integration default-off. The area and precision goals can be met, but not together with the per-dataset non-regression requirement; blanket score-to-mask replacement is rejected. |
| **V4.2** | Real-candidate selector and localization recalibration | ✅ Prototype complete; promotion gate failed | Reconstructed category-free candidate pools for 144 MVTec and 1,200 exposed VisA images, added 22 normal/posterior/consensus features, paired gain arbitration, and an evidence-proposal localization envelope. Dataset-excluded Spearman is `0.7481/0.5650` with MAE `0.0835/0.0854`; search recall is `0.9191/0.9682`. However, MVTec Dice changes `0.5889 -> 0.5786`, VisA remains `0.1593`, and VisA conformal coverage is only `0.7601`. A committed rerun reproduces the model, full metrics, report, and calibration-manifest hashes exactly in `367.09` seconds. | Keep both candidate arbitration and localization default-off. Absolute calibration transfers, but within-image ranking does not; utility and per-dataset coverage gates fail. |
| **V4.2b** | Within-image pair/list ranking with broader exposed development | ✅ Prototype complete; promotion gate failed | Added all-pairs antisymmetric list ranking, absolute pair/list context, a dual-head `+0.01` expected-IoU agreement guard, and isolated Kodytek as a third exposed source. Across 1,356 images, MVTec improves `0.5889 -> 0.5934`, while VisA (`0.1593`) and Kodytek (`0.3164`) remain unchanged. Macro gain is only `+0.0015`; pairwise coverage is `0.3462/0.9631/0.9182` for MVTec/VisA/Kodytek. All `280` tests pass. | Keep V4.2b default-off. The dual-head guard prevents regression, but utility and cross-domain risk calibration fail. Do not tune further on the same three exposed sources. |
