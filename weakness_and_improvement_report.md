...
| **V4.2c** | Broader cross-domain selective-risk calibration | ⬜ Blocked on additional exposed datasets | Retain V4.2b contracts, but estimate domain-robust selection risk from at least two more independent labeled industrial datasets. No category-specific thresholds and no reuse of locked benchmarks. | Re-run leave-dataset-out gates only after broader calibration support exists; require macro gain `>= 0.01`, every-dataset regression `<= 0.02`, coverage `0.85-0.95`, and search recall `>= 0.85`. |
