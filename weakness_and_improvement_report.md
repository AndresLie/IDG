...
| **V4.2b** | Within-image pair/list ranking with broader exposed development | 🟡 Next active sprint | Replace absolute-IoU-first arbitration with group-aware pair/list ranking and add a third exposed dataset so cross-dataset residuals are estimable rather than inferred from one source. Preserve the V3 mask as fallback and report localization recall jointly with search area. | Require macro Dice gain `>= 0.01`, every-dataset regression `<= 0.02`, every-dataset coverage `0.85-0.95`, and every-dataset search recall `>= 0.85`. |
