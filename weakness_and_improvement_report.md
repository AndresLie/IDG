...
| **V4.2** | Real-candidate selector and localization recalibration | 🟡 Next active sprint | Refit IoU/precision/recall heads on real multi-dataset candidates; use candidate-level normal-boundary burden, compact-posterior support, spatial-prior containment, and multi-proposal localization. | Require expected-IoU Spearman `>= 0.40`, MAE `<= 0.12`, conformal coverage `0.85-0.95`, search recall `>= 0.85`, and no dataset Dice regression above `0.02`. |
